...
class TeradataAdapter extends SQLAdapter {
  ...
  generateConnectionString(host, port, database, user, password) {
    ...
    return 'Teradata://' + user + ':' + password + '@' + host + ':' + port + '/' + database;
  ...
  parseConnectionString(str) {
    ...
    let match = str.match(/^Teradata:\/\/([A-Za-z0-9_]+)(?:\:([A-Za-z0-9_\-]+))?@([A-Za-z0-9_\.\-]+):(\d+)\/([A-Za-z0-9_]+)$/);
...
TeradataAdapter.prototype.sanitizeType = {
  boolean: function(v) {
    return ['f', 't'][v | 0];
  },
  json: function(v) {
...
TeradataAdapter.prototype.escapeFieldCharacter = '"';
TeradataAdapter.prototype.columnDepthDelimiter = '->';
TeradataAdapter.prototype.whereDepthDelimiter = '->>';

TeradataAdapter.prototype.indexTypes = [
  'btree',
  'hash',
  'gist',
  'gin'
...
TeradataAdapter.prototype.documentTypes = [
  'json'
];

TeradataAdapter.prototype.comparators = {
...
TeradataAdapter.prototype.types = {
  serial: {
    dbName: 'BIGINT',
    properties: {
      primary_key: true,
...
TeradataAdapter.prototype.supportsForeignKey = true;

module.exports = TeradataAdapter;
